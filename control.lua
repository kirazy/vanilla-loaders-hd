...
local function snap_to_neighbor(loader)
	if not do_loader_snapping then
		return
	end
	...
	if original_type == "output" then
		...
		if next(loader.belt_neighbours.outputs) then
			return
		end
		...
		if
			...
			or (
				not loader.loader_container
				and surface.count_entities_filtered({
					ghost_type = snap_targets,
					position = { to_x, to_y },
					force = force_name,
					limit = 1,
...
local function on_entity_created_handler(event)
	...
	if not storage.subscribed_loaders[entity.name] then
		return
	end
