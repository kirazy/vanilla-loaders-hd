...
local function create_explosion_prototype_with_particles(loader_name, mask_tint, base_tint)
	...
	for particle_name, index in pairs(particle_mapping) do
		...
		if particle.pictures and particle.pictures.sheet then
		...
		if
			...
			and explosion.created_effect.action_delivery
			and explosion.created_effect.action_delivery.target_effects
			and explosion.created_effect.action_delivery.target_effects[index]
		then
			explosion.created_effect.action_delivery.target_effects[index].particle_name = particle.name
		end
	end
...
local function set_entity_sprites_and_properties(loader, transport_belt, mask_tint, base_tint, heating_energy)
	...
	if feature_flags.freezing then
		if heating_energy ~= nil then
			loader.heating_energy = heating_energy
		else
			---@type flib_data_util
			---@diagnostic disable-next-line: unresolved-require
			local data_utils = require("__flib__.data-util")
			local value, _ = data_utils.get_energy_value(transport_belt.heating_energy)
			loader.heating_energy = tostring(15 * (value or 0)) .. "W"
		end
	end
...
function api.set_loader_item_order_from_belt(loader, transport_belt)
	...
	if type(transport_belt) == "string" then
		...
		if not transport_belt_item then
			debug_error(
				"(set_loader_item_order_from_belt) the transport belt item with the name "
					.. transport_belt
					.. " does not exist."
			)
		end
	...
	loader_item.order =
		string.gsub(string.gsub(transport_belt_item.order or "", "^[a-z]", sort_prefix), "transport%-belt", "loader")
end
...
---@field heating_energy? data.Energy The energy cost of heating the loader. If omitted, defaults to a multiple of the reference belt heating energy. Only used if loaded with Factorio: Space Age.
...
function api.create_loader(name, belt_name, parameters)
	if not name then
		debug_error("(create_loader) loader_name is nil.")
	end
	if not belt_name then
		debug_error("(create_loader) transport_belt_name is nil.")
	end
	if not parameters then
		debug_error("(create_loader) parameters is nil.")
	end
	...
	if not parameters.base_tint then
		debug_log("(create_loader) parameters.base_tint is nil for loader '" .. name .. "', using default (black).")
	end

	if not parameters.ingredients then
		debug_log("(create_loader) parameters.ingredients is nil for loader '" .. name .. "', using default recipe.")
	end
	...
	if not belt_item or not belt_entity then
		debug_error(
			"(create_loader) the belt '" .. belt_name .. "' does not exist, loader '" .. name .. "' cannot be created."
		)
		return
	end
	...
	if loader_recipe then
		...
		loader_recipe.categories = belt_recipe and belt_recipe.categories or loader_recipe.categories
		...
		local recipe = {
			...
			categories = belt_recipe and belt_recipe.categories,
	...
	---@type data.Loader1x2Prototype?
	local loader_entity
	if data.raw.loader[name] then
		loader_entity = data.raw.loader[name]

		set_entity_sprites_and_properties(
			loader_entity,
			belt_entity,
			parameters.mask_tint,
			parameters.base_tint,
			parameters.heating_energy
		)
		loader_entity.hidden = nil
	elseif data.raw.loader["loader"] then
		loader_entity = util.copy(data.raw.loader["loader"])
		assert(loader_entity.minable, "Base loader entity is missing minable results: " .. serpent.block(loader_entity))
	...
	if parameters.previous_tier and data.raw.loader[parameters.previous_tier] then
		data.raw.loader[parameters.previous_tier].next_upgrade = name
	end
	if parameters.next_tier and loader_entity then
		loader_entity.next_upgrade = parameters.next_tier
	end
